<template>
  ...
  <transition name="leaveDetails">
    <div class="leaveDetails-container">
      ...
      <cube-scroll :data="items" :options="options">
        ...
        <div class="opinion"  v-show="contentdetails.taskState === '5'">
          <cube-button class="view" :light="true" v-on:click="open('确认同意')">同意</cube-button>
          <cube-button class="view" :light="true" v-on:click="open('确认拒绝')">拒绝</cube-button>
        </div>
    ...
    <div class="opinion-content" v-show="showopinion">
      ...
      <cube-button  class="opinionBtn" :light="true" @click="add">{{msg}}</cube-button>
    </div>
    </div>
  </transition>
</template>
...
export default {
  ...
  methods: {
    ...
    getPersonalList: function () {
      let data = {
        id: this.$store.getters.vacationId,
