   ...
   <div class="details">
       <div class="name">
           ...
           <div class="nametext">
             ...
             <p class="wait-p" v-show="contentdetails.taskState === '5'"> 等待<span> {{contentdetails.waitOpinion}}</span> 审批中 </p>
             <p class="wait-p" v-show="contentdetails.taskState === '1'">等待 {{contentdetails.waitOpinion}} 审批中</p>
             <p class="wait-p" v-show="contentdetails.taskState === '8'">等待 {{contentdetails.waitOpinion}} 审批中</p>
           </div>
...
export default {
  ...
  methods: {
    ...
    goRouter (item) {
      console.log(item)
      // 特别注释一下 0 查看申请
      this.$router.push({name: 'otherdetail', params: {applicantId: item.applicantId, type: 0, name: this.contentdetails.name, taskType: item.type}})
    }
  },
  created() {
    this.newTime()
