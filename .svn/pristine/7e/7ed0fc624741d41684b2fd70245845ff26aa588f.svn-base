<template>
  <div class="vacation-wrapper">
    <cube-form :model="model" :immediate-validate="false" @validate="validateHandler" @submit="submitHandler"  @reset="resetHandler" ref="cubeForm">
      <cube-form-group>
        <div class="title header-title" v-show="headerTitleShow">
          <span class="header-title-left header-title-item">{{nowDate}}年{{headerTitle.taskType}}已请<span style="color:#00f">{{headerTitle.number}}</span>次<span style="color: red; padding-left: 2px">{{headerTitle.timeSum}}</span>天</span>
      ...
      <cube-form-group>
        ...
        <cube-form-item :field="fields[1]">
          <div @click="showDatePicker('start')" class="btn">{{model.startTime || '请选择开始时间'}}</div>
          <date-picker ref="datePicker" @select="dateSelectHandler" :dateTypes="dateTypes"></date-picker>
        </cube-form-item>
        <cube-form-item :field="fields[2]">
          <div @click="showDatePicker('end')" class="btn">{{model.endTime || '请选择结束时间'}}</div>
          <date-picker ref="datePicker" @select="dateSelectHandler" :dateTypes="dateTypes"></date-picker>
        </cube-form-item>
        <!-- 时长 -->
...
<script>
  ...
  export default {
    data () {
      return {
        ...
        fields: [{
          ...
          props: {
            action: {
              target: 'http://192.168.199.98:8086/process/Resources/save'
            },
            max: 6
          },
          messages: {
    ...
    methods: {
      ...
      showDetail() {
        this.$router.push({name: 'allvacation', params: {type: this.model.selectValue, url: '/vacation/index'}})
      },
      // 查看请假次数及时间
